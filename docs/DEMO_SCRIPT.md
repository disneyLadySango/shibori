...
## 0:45–1:10 — Multiple purposes and one recommendation

**Screen:** Add `量子力学が面白そう`, enter `深い集中を30分`, and request a recommendation.

> 学習目的は複数持てます。それぞれの到達状態、現在地、理解状態、抜けは混ざりません。GPT-5.6は今使える集中資源も踏まえて一つだけおすすめしますが、量子力学を選び続けても失敗にはしません。最終決定は学習者のものです。

## 1:10–1:30 — Can, path, current position
...
## 1:30–1:50 — One focus and material projection
...
## 1:50–2:12 — Check, gap, and choice
...
> Codexとは、まず原点の言葉、ユーザーストーリー、AC、テスト仕様を合意しました。Slice AとBを合計22の承認仕様からテスト先行で作り、GPT-5.6のstrict schema、UI、保存、production build、ブラウザ確認まで再構築しました。
