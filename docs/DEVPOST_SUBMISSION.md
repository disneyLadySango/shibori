...
学習目的は複数持てます。それぞれの到達状態、現在地、理解状態、確認結果、抜けを混ぜずに保存します。GPT-5.6は全目的と今使える集中資源から次の学習目的を一つだけ理由つきですすめますが、自動決定はしません。学習者は別の目的を選べ、切り替えた後も各目的の続きへ戻れます。
...
目的間の配分では、すべての学習目的を同時に渡し、一つの既存目的ID、理由、判断に使った事実だけをstrict schemaで受け取ります。期限や実用性がないことだけで好奇心の学びを低く扱わない制約をプロンプトとテストの両方で守ります。
...
承認済みロードマップに沿って、状況変化による配分更新、学習目的の保留・終了、配分の振り返り、生活リズムに合わせた配信、PDF・画像OCRへ拡張します。
